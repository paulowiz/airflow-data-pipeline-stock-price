...
def stock_market():
    ...
    @task.sensor(poke_interval=3, timeout=300, mode='poke')
    def is_api_available()-> PokeReturnValue:
        api = BaseHook.get_connection('stock_api')
        url = f"{api.host}{api.extra_dejson['endpoint']}"
        response = requests.get(url, headers=api.extra_dejson['headers'])
        conditiion = response.json()['chart']['error'] is None
        return PokeReturnValue(is_done=conditiion, xcom_value=url)
    ...
    get_fromatted_csv = PythonOperator(
    ...
    is_api_available() >> get_stock_prices >> store_prices >> format_prices >> get_fromatted_csv >> load_to_dw
